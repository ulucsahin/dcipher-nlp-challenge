...
class JSONManager(object):
    ...
    def __init__(self, config, augmenter):
        ...
        if config.use_existing_data:
            self.read_test_train_data(config.train_data_path, config.test_data_path)
        else:
            self.augmenter = augmenter
            self.read_json(config.json_path)
            self.shuffle()
            ...
            self.split_test_train(config.train_ratio, config.split_method)
            self.pad_sentences(50, 300)
            if(config.augment):
                self.augment_train_data()
            self.create_train_test_jsonfile()
    ...
    def count_labels(self):
        ...
        print(f"Train Num Material: {counts_test[0]}")
        print(f"Train Num Chemistry: {counts_test[1]}")
